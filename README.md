...
| 10 | `Reveal.tsx` | `checkout.buildCheckoutOrder` | in-app `Checkout.tsx` | The cart, built from the recommendation **on screen** — the cheaper plan if they swapped. `handoff.buildHandoffUrl` stays on the `<a>` underneath as the fallback when no `onClaim` is wired |
