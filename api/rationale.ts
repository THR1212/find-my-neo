/**
 * POST /api/rationale — the whole run in, the two sentences under the price out.
 *
 * Thin wrapper over `api/_lib/reasonService.ts`, which the Vite dev server also mounts.
 *
 * Its own route for latency placement, not tidiness. /api/questions gates the first question
 * screen, so anything added there delays a screen someone is waiting on; reasons are not
 * needed until the reveal, 30s+ away and already waiting on Neo's generator. See
 * reasonService's header for the fixed/generated split it preserves.
 *
...
import { handleRationale } from "./_lib/rationaleService.js";
...
export default async function handler(req: NodeReq, res: NodeRes): Promise<void> {
  ...
  /* Unlike the other model routes this takes a whole object — the description, the answers,
     and the plan already chosen — so the entire parsed body is forwarded. */
  let payload: unknown;
  try {
  ...
  const { status, body } = await handleRationale(payload as never, sid);
