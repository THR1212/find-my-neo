...
Everything starts at `submitDescription` in `src/App.tsx`. It fires three requests **in
...
Only 1 and 2 are model calls. 3 is **Neo's** generator, not ours. 4 is DomScan.
...
- **All 20 `because` strings** in `features.ts` (11 mail, 9 site). Feature *names* are Neo's; the reason we give
  you for caring is hand-written and identical for everyone who matches. This is the biggest
  remaining item, and it should follow the same three-layer pattern `questionService` proves:
  deterministic matching, Neo's verbatim name, only the reason generated, degrading to the
  fixed string.
