/**
 * POST /api/reasons — free text in, per-feature "why this matters to you" clauses out.
 *
...
import { handleReasons } from "./_lib/reasonService.js";
...
export default async function handler(req: NodeReq, res: NodeRes): Promise<void> {
  ...
  const { status, body } = await handleReasons(businessText, sid);
